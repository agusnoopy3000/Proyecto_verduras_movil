...
class CartViewModel(
    ...
    fun addToCart(productId: String) {
        viewModelScope.launch {
            ...
            product?.let { foundProduct ->
                ...
                // Registrar evento de agregar al carrito en Analytics
                AnalyticsManager.logAddToCart(
                    itemId = foundProduct.id,
                    itemName = foundProduct.nombre,
                    quantity = 1,
                    price = foundProduct.precio
                )
                
                // Actualizar info del carrito en Crashlytics
                CrashlyticsManager.setCartInfo(
                    itemCount = _cartState.value.items.sumOf { it.qty },
                    totalValue = _cartState.value.total
                )
            }
        }
    }
    ...
    fun confirmOrder(
        ...
        viewModelScope.launch {
            ...
            // Registrar inicio de checkout en Analytics
            val cartTotal = _cartState.value.total
            val itemCount = _cartState.value.items.sumOf { it.qty }
            AnalyticsManager.logBeginCheckout(cartTotal, itemCount)
            CrashlyticsManager.logCheckoutStarted()
            
            // Iniciar trace de Performance para checkout
            PerformanceManager.startCheckoutTrace(itemCount)
            ...
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    // Sincronizar pedido con Firebase Firestore (para que admin vea todos los pedidos)
                    try {
                        FirestoreService.savePedido(localPedido)
                        Log.d("CartViewModel", "Pedido sincronizado con Firestore: ${localPedido.id}")
                    } catch (e: Exception) {
                        Log.w("CartViewModel", "Error sincronizando pedido con Firestore", e)
                    }
                    
                    // Limpiar carrito
                    _cartState.value = CartState()
                    
                    // Registrar compra exitosa en Analytics
                    AnalyticsManager.logPurchase(
                        orderId = orderResponse.id.toString(),
                        totalValue = orderResponse.total,
                        itemCount = orderItems.size,
                        paymentMethod = "transferencia" // Ajustar según el método de pago usado
                    )
                    CrashlyticsManager.logCheckoutCompleted(orderResponse.id.toString())
                    
                    // Detener trace de Performance (checkout exitoso)
                    PerformanceManager.stopCheckoutTrace(true, orderResponse.id.toString())
                    ...
                    CrashlyticsManager.recordNetworkError(
                        endpoint = "/orders",
                        errorCode = response.code(),
                        errorMessage = response.message()
                    )
                    
                    // Detener trace de Performance (checkout fallido)
                    PerformanceManager.stopCheckoutTrace(false)
                ...
                // Registrar excepción en Crashlytics
                CrashlyticsManager.recordException(e, "Error al crear pedido")
                
                // Detener trace de Performance (checkout con excepción)
                PerformanceManager.stopCheckoutTrace(false)
